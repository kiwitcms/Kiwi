...
- Fix render_to_string (Chenxiong Qi)
- Replace dict.iteritems calls with dict.items in templates (Chenxiong Qi)
- Fix settings value in doc for installing with Apache (virtualenv) (Chenxiong Qi)
- Fix plans and cases search (Chenxiong Qi)
- Fix wrong name from_rlan (Chenxiong Qi)
- Port to Python 3 (Chenxiong Qi)
- Run tests with multiple versions of Python and Django (Chenxiong Qi)
- Replace MySQL-python with PyMySQL (Chenxiong Qi)
- Read version from a dedicated file (Chenxiong Qi)
- Move to django 1.10 (Chenxiong Qi)
- Deprecated django-celery and use Celery instead (Chenxiong Qi)
- Upgrade kobo to 0.7.0 (Chenxiong Qi)
- Remove djqpid (Chenxiong Qi)
- Omit more files from coverage report (Chenxiong Qi)
- Drop django-pagination and use django.core.paginator (Chenxiong Qi)
- Reorganize code layout (Chenxiong Qi)
- Fix E722 do not use bare except (Chenxiong Qi)
- Add URL mapping names for resetting password (Chenxiong Qi)
- Refactor datatable response (Chenxiong Qi)
- Fix deprecated except statement (Chenxiong Qi)
- Upgrade kerberos to 1.2.5 (Chenxiong Qi)
- Remove unused switch tag (Chenxiong Qi)
- Upgrade django-tinymce to 2.6.0 (Chenxiong Qi)
- kobo can be installed from PyPI (Chenxiong Qi)
- Specify versions for dependent packages (Chenxiong Qi)
- Replace REQUEST properly in run get and edit templates (Chenxiong Qi)
- Replace REQUEST with POST in cases.views.new (Chenxiong Qi)
- Replace REQUEST with GET in cases.views.search (Chenxiong Qi)
- Replace REQUEST properly in cases.views.clone (Chenxiong Qi)
- Do not load unused tag in printable template (Chenxiong Qi)
- Replace REQUEST with POST in cases.views.printable (Chenxiong Qi)
- Replace REQUEST with POST in cases.views.export (Chenxiong Qi)
- Replace REQUEST with GET and POST in part case views (Chenxiong Qi)
- Replace REQUEST with POST in cases.views.automated (Chenxiong Qi)
- Replace REQUEST with GET in cases.views.ajax_search (Chenxiong Qi)
- Replace REQUEST with POST in cases.views.edit (Chenxiong Qi)
- Remove unused obj2dict.py (Chenxiong Qi)
- Replace GET_CONFIRMED_CASES SQL with ORM query (Mr. Senko)
- Replace GET_CASERUNS_COMMENTS SQL with Django ORM query (Mr. Senko)
- Add test for core.helper.comments.add_comment (Chenxiong Qi)
- Split testruns/tests.py (Chenxiong Qi)
- Replace GET_CASERUNS_BUGS with ORM query (Mr. Senko)
- Replace GET_RUN_BUG_IDS SQL with ORM query (Mr. Senko)
- Replace REQUEST with GET and POST in testruns view method (Chenxiong Qi)
- Replace REQUEST with POST in testruns.views.edit (Chenxiong Qi)
- Replace REQUEST with POST in testrun update_case_run_text (Chenxiong Qi)
- Replace REQUEST with POST in testruns.views.remove_case_runs (Chenxiong Qi)
- Replace REQUEST with GET in testruns.views.bug (Chenxiong Qi)
- Replace REQUEST with POST in testruns.views.AddCasesToRunView (Chenxiong Qi)
- Replace REQUEST with GET in testruns.views.export (Chenxiong Qi)
- Remove deprecated env_value action (Chenxiong Qi)
- Replace REQUEST with GET in testruns.views.env_value (Chenxiong Qi)
- Replace REQUEST with POST in testruns.views.order_case (Chenxiong Qi)
- Do not create duplicate tester user in tests (Chenxiong Qi)
- Clean tests (Chenxiong Qi)
- Replace REQUEST with GET in cc view (Chenxiong Qi)
- Replace REQUEST with GET in search runs (Chenxiong Qi)
- Replace REQUEST in new and clone view method in testruns (Chenxiong Qi)
- Replace GET_BUG_COUNT SQL with ORM query (Mr. Senko)
- Replace STATS_CASERUNS_STATUS SQL with ORM query (Chenxiong Qi)
- Replace REQUEST with GET in testruns.views.get (Chenxiong Qi)
- New base test case class BaseCaseRun (Chenxiong Qi)
- Fix tags search and hint while adding tags to test cases in TP (Mr. Senko)
- Simplify strip_parameters() function and add tests for it (Mr. Senko)
- Replace REQUEST with GET in core.ajax.info (Chenxiong Qi)
- Fix: consider nonexisting product_id when get object info (Chenxiong Qi)
- Replace REQUEST with POST in TestCaseUpdateActions (Chenxiong Qi)
- Replace REQUEST with GET in core.ajax.form (Chenxiong Qi)
- Replace REQUEST with POST in core.ajax.update_case_run_status (Chenxiong Qi)
- Replace REQUEST with POST in core.ajax.update (Chenxiong Qi)
- Replace REQUEST with POST in core.ajax.comment_case_runs (Chenxiong Qi)
- Remove unused CLONE_ENV_VALUES SQL statement (Mr. Senko)
- Remove unused doctest (Chenxiong Qi)
- Remove unused del action of properties (Chenxiong Qi)
- Refactor view management.views.environment_properties (Chenxiong Qi)
- Replace REQUEST with GET in management.views.environment_group_edit (Chenxiong Qi)
- Refactor environment_groups (Chenxiong Qi)
- Remove double assignment (Mr. Senko)
- Update texts and wording (Mr. Senko)
- Fix typo in class name (Mr. Senko)
- Update wording in template (Mr. Senko)
- Add tests for auth.models (Chenxiong Qi)
- Replace REQUEST with GET or POST in auth.views (Chenxiong Qi)
- Fix: use POST to delete comment from reviewing cases (Chenxiong Qi)
- Replace REQUEST with GET or POST in profiles.views.bookmark (Chenxiong Qi)
- Fix: avoid modifying dict while iterating over it (Chenxiong Qi)
- Refactor core.views.search (Chenxiong Qi)
- Replace REQUEST with GET in core.views.search (Chenxiong Qi)
- Fix: show comments count correctly in a run page (Chenxiong Qi)
- Clean removing a comment (Chenxiong Qi)
- Declare comments.views.post requires POST explicitly (Chenxiong Qi)
- Remove unused code of comments (Chenxiong Qi)
- Add tests for view method testcases.views.tag (Chenxiong Qi)
- Replace REQUEST with GET in testcases.views.plan (Chenxiong Qi)
- Replace REQUEST with GET in testcases.views.bug (Chenxiong Qi)
- Refactor plan clone view method (Chenxiong Qi)
- Replace REQUEST with POST in view testcases.views.category (Chenxiong Qi)
- Tests for operating component of cases (Chenxiong Qi)
- Use POST to add components in case page (Chenxiong Qi)
- Do not list migrations in coverage report (Chenxiong Qi)
- Tests for CaseActions (Chenxiong Qi)
- Use JsonResponse in delete_file view (Chenxiong Qi)
- Replace REQUEST with GET in delete file view (Chenxiong Qi)
- Upgrade django-debug-toolbar to 1.7 (Chenxiong Qi)
- Replace REQUEST with POST in tcms.core.files.upload_file (Chenxiong Qi)
- Refactor test utility functions (Chenxiong Qi)
- Fix: allow NULL to TestCaseAttachment.case_run (Chenxiong Qi)
- Replace REQUEST with GET in tcms.core.utils.clean_request (Chenxiong Qi)
- Handle GET and POST properly in testplans.views.cases (Chenxiong Qi)
- Replace REQUEST with GET in testplans.views.text_history (Chenxiong Qi)
- Handle GET and POST properly in testplans.views.clone (Chenxiong Qi)
- Replace REQUEST with POST in BaseAction (Chenxiong Qi)
- Replace REQUEST with POST in ComponentActions (Chenxiong Qi)
- Replace REQUEST with POST in CategoryActions (Chenxiong Qi)
- Fix traceback when removing test case tags and add test for it (Mr. Senko)
- Replace deprecated request.REQUEST with request.POST (Mr. Senko)
- Simplify removal SQLs for testcases (Mr. Senko)
- Remove some occurences of deprecated request.REQUEST (Mr. Senko)
- Update installation steps for RHEL6 (Mr. Senko)
- Compare PKs with what was written in DB, not hard-coded (Mr. Senko)
- Update pre_check_product() and pre_process_ids() to include long (Mr. Senko)
- Don't use unique test object names (Mr. Senko)
- Remove tcms.core.migrations.0001_django_comments__object_pk (Mr. Senko)
- Add test and refactor SQL for XML-RPC testcase.link_plan() (Mr. Senko)
- Add tests and SQL->ORM refactor for TestPlan.delete_case() (Mr. Senko)
- Add tests and refactor SQL for XML-RPC TestCase.unlink_plan (Mr. Senko)
- Just use get_or_create() without try-except (Mr. Senko)
- Exclude test files from coverage report (Mr. Senko)
- Convert setUpClass to setUpTestData and drop tearDownClass() (Mr. Senko)
- Refactor TC_REMOVE_CC SQL into ORM (Mr. Senko)
- Refactor TP_CLEAR_ENV_GROUP and TP_ADD_ENV_GROUP SQLs into ORM. (Mr. Senko)
- Add test for xmlrpc.testplans.update (env_group update) (Mr. Senko)
- Fix quotes in testruns sqls.py (Mihail Mihaylov)
- Test setup.py and manage.py with flake8 and fix few errors (Mr. Senko)
- Don't install files under /etc from setup.py (Mr. Senko)
- Add Travis-CI and Coveralls badges. Fix #163 (Mr. Senko)
- Fix deprecated commit_unless_managed (Chenxiong Qi)
- Fix TestCaseTagFactory.user (Chenxiong Qi)
- Enable Coveralls.io (Mr. Senko)
- Ensure the content-type of uploaded XML file (Chenxiong Qi)
- Fix test case in test_product (Chenxiong Qi)
- Let testplans tests run again (Chenxiong Qi)
- Fix: space should not be in username created by UserFactory (Chenxiong Qi)
- Fix two small pep8 errors (Chenxiong Qi)
- Clean tests in integration app (Chenxiong Qi)
- Remove legacy test case (Chenxiong Qi)
- Let testcases tests run again (Chenxiong Qi)
- Use version from module, not txt file (Mr. Senko)
- Remove unnecessary error assertions in xmlrpc tests (Chenxiong Qi)
- Simplify xmlrpc utils tests (Chenxiong Qi)
- Simplify xmlrpc Fault code assertion in xmlrpc tests (Chenxiong Qi)
- Use mock user object in tests in test_user.py (Chenxiong Qi)
- Fix duplicate AssertMessage (Chenxiong Qi)
- Fix xmlrpc tests (Chenxiong Qi)
- Disable more verbose mode while running tests (Chenxiong Qi)
- Ignore directories while making tags (Chenxiong Qi)
- Add documentation about installation with Apache and virtualenv (Mr. Senko)
- Move tests into tcms (Chenxiong Qi)
- Fix tests in tcms/core/ (Chenxiong Qi)
- Ignore more files (Chenxiong Qi)
- Use factory_boy to create fixtures (Chenxiong Qi)
- Replace nose with pytest to run tests (Chenxiong Qi)
- Fix testplans tests and all pass (Chenxiong Qi)
- Remove read_file from setup.py (Chenxiong Qi)
- Dont install kobo from RPM in provision (Chenxiong Qi)
- Run checks with new target in Makefile (Chenxiong Qi)
- Clean Makefile (Chenxiong Qi)
- Add docker and rpmutils provision steps in Vagrantfile (Chenxiong Qi)
- Simplify install section (Chenxiong Qi)
- Do not try to generate debuginfo (Chenxiong Qi)
- Fix: build doc package by default and drop apidoc build (Chenxiong Qi)
- Fix deprecated url in spec (Chenxiong Qi)
- Fix: add missing changelog to spec and bump to right version (Chenxiong Qi)
- Refine Python package (Chenxiong Qi)
- Use bztar as the default type of tarball (Chenxiong Qi)
- Use singular in group name (Chenxiong Qi)
- Remove unnecessary initial data for profiles app (Chenxiong Qi)
- Update help strings of clone case form and update docs (Mr. Senko)
- Bug fix: bring back plan filtering code deleted in d4f815f (Mr. Senko)
- Use make test && flake8 in Travis-CI (Mr. Senko)
- Make Travis-CI notify only on changes (Mr. Senko)
- Drop support for Python 2.6 (Mr. Senko)
- Install kobo from source (Chenxiong Qi)
- Document how to install Nitrate with Docker and Google Cloud Engine (Mr. Senko)
- Document how to install and serve Nitrate with Gunicorn (Mr. Senko)
- Include LICENSE text in documentation (Mr. Senko)
- Fix typo (Mr. Senko)
- Document how to start Nitrate locally for hacking (Mr. Senko)
- Cleanup after update to Django-1.8.14 (Mr. Senko)
- Update doc to use latest Vagrantfile (Chenxiong Qi)
- Refactor the way to run tests (Chenxiong Qi)
- Fix Normal Search (Chenxiong Qi)
- Don't use RawSQL in migrations (Mr. Senko)
- Remove unused imports to make flake8 happy (Mr. Senko)
- Upgrade to django-tinymce 2.4.0 and remove left over tiny_mce.js (Mr. Senko)
- Update CHANGELOG with missing info for 3.8.18 (Mr. Senko)
- Add unique to TestCaseBugSystem.name (Chenxiong Qi)
- Dont pass max_length TestRun.estimated_time (Chenxiong Qi)
- Add initial data (Chenxiong Qi)
- Remove max_length from IntegerField (Chenxiong Qi)
- Refine contribution.rst (Chenxiong Qi)
- Refine distribution package (Chenxiong Qi)
- Tiny gramatic correction (Matthias Cavigelli)
- Fixing link (Matthias Cavigelli)
- Fix code style errors checked by flake8 (Mr. Senko)
- Fix database name in settings.devel (Chenxiong Qi)
- Refine dependency files and Vagrantfile (Chenxiong Qi)
- Update README (Chenxiong Qi)
- Refactor update data of updating cases reviewer (Chenxiong Qi)
- Refactor JS function of updating cases default tester (Chenxiong Qi)
- Refactor post data of updating cases priority (Chenxiong Qi)
- Remove duplicate code of updating case status (Chenxiong Qi)
- Fix removed get_profile method (Chenxiong Qi)
- Reduce SQL queries against case text (Chenxiong Qi)
- Set file upload directory in devel.py (Chenxiong Qi)
- Update changelog (Chenxiong Qi)
- Remove raw SQL migrations and initial schema and data (Chenxiong Qi)
- Add migration for django_comments (Chenxiong Qi)
- Upgrade Django to 1.8.14 (Chenxiong Qi)
