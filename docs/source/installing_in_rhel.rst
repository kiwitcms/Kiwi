...
Installing KiwiTestPad on RHEL6 with Apache and MySQL
=====================================================
...
install this project with python setup.py::
  ...
  cd [kiwi_download_path]/Kiwi
  sudo python setup.py install

Initialize database
-------------------

Database is required by KiwiTestPad (and all of Django apps). Django ORM supports
...
Update settings/product.py::
    ...
    DATABASES = {
        'default': {
            ...
            'NAME': 'kiwi',                       # Or path to database file if using sqlite3.
...
First please go to KiwiTestPad root path, it's different based on your current OS.

Like on RHEL6.3, the root path is located in::

    /usr/lib/python2.7/site-packages/kiwi-*/tcms

As we plan to deploy an example server for KiwiTestPad, we can use product.py as the
...
Create upload dir and change dir own & group to apache::
    ...
    sudo mkdir -p /var/kiwi/uploads
    sudo chown apache:apache /var/kiwi/uploads

Collect static files
~~~~~~~~~~~~~~~~~~~~

The default directory to store static files is `/var/kiwi/static`, you can
modify it by changing `STATIC_ROOT` setting in
`/path/to/kiwi/tcms/settings/product.py`.
...
I presume that the conf file is named kiwi-httpd.conf.
