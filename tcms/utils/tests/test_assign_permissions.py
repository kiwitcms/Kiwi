...
class TestAssignDefaultGroupPermissions(TestCase):
    ...
    def test_administrator_has_all_permissions(self):
        ...
        self.assertEqual(Permission.objects.all().count(),
    ...
    def test_tester_has_kiwi_apps_permissions(self):
        ...
        for app_name in kiwi_apps:
            ...
            self.assertEqual(Permission.objects.filter(
                content_type__app_label__contains=app_name).count(),
                self.tester.permissions.filter(
                content_type__app_label__contains=app_name).count())

        self.assertTrue(self.tester.permissions.filter(
            pk__in=Permission.objects.filter(
                content_type__app_label__contains='attachments')).exists())
        self.assertEqual(Permission.objects.filter(
            content_type__app_label__contains='attachments').count(),
            self.tester.permissions.filter(
            content_type__app_label__contains='attachments').count()+1)

        self.assertFalse(self.tester.permissions.filter(
