...
class BitBucketAPI():
    ...
    def create_issue(self, data):
        ...
        return self._request("POST", url, headers=self.headers, auth=self.auth, json=data)
    ...
    def add_comment(self, issue_id, comment):
        url = '{0}{1}{2}{3}'.format(self.endpoint_url, '/issues/', issue_id, '/comments/')
        return self._request("POST", url, headers=self.headers, auth=self.auth, json=comment)

    def get_comments(self, issue_id):
        url = '{0}{1}{2}{3}'.format(self.endpoint_url, '/issues/',
                                    issue_id, '/comments?sort=-updated_on')
        return self._request("GET", url, headers=self.headers, auth=self.auth)

    def delete_comment(self, issue_id, comment_id):
        url = '{0}{1}{2}{3}{4}'.format(self.endpoint_url, '/issues/',
    ...
    def _construct_endpoint_url(api_version, url):
        ...
        endpoint_url = "{0}/{1}/{2}/{3}/{4}".format(base_url,
                                                    api_version,
                                                    'repositories',
                                                    workspace,
                                                    repository)
...
class BitBucket(IssueTrackerType):
    ...
    def _rpc_connection(self):
        return BitBucketAPI(self.bug_system.base_url,
                            api_username=self.bug_system.api_username,
                            api_password=self.bug_system.api_password)

    def is_adding_testcase_to_issue_disabled(self):
        return not (self.bug_system.base_url and
                    self.bug_system.api_username and
