...
def forwards_add_initial_data(apps, schema_editor):
    ...
    group_model.objects.using(database).bulk_create(
        [
    ...
    site_model.objects.using(database).create(name="localhost", domain="127.0.0.1:8000")
...
def reverse_remove_initial_data(apps, schema_editor):
    ...
    site_model = apps.get_model("sites", "Site")
    site_model.objects.using(database).filter(name="localhost").delete()
...
def forwards_add_default_perms(apps, schema_editor):
    ...
    admin = group_model.objects.using(database).get(name="Administrator")
    all_perms = permission_model.objects.using(database).all()
