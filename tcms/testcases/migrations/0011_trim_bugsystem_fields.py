...
def forwards_store_data(apps, schema_editor):
    ...
    for bug_system in bug_system_model.objects.all():
        file_name = '/tmp/kiwitcms-testcases-migration-0011-BugSystem-%d' % bug_system.pk

        with open(file_name, 'w') as outfile:
            print('>>> FORWARDS: dumping BugSystem', bug_system.pk, bug_system.name)
...
def backwards_restore_data(apps, schema_editor):
    bug_system_model = apps.get_model('testcases', 'BugSystem')
    file_name = ''  # nosec:B108

    for file_name in glob.glob('/tmp/kiwitcms-testcases-migration-0011-BugSystem-*'):
        print('<<< REVERSE: restoring from', file_name)

        with open(file_name, 'r') as infile:
            data = json.load(infile)
            print('<<< REVERSE: restoring', data)
