...
@require_POST
def build_cases_search_form(  # pylint: disable=missing-permission-required
    request, populate=None, plan=None
):
    ...
    if action in TESTCASE_OPERATION_ACTIONS:
        ...
        items_per_page = request.session.get(
            "items_per_page", settings.DEFAULT_PAGE_SIZE
        )
        search_form = SearchCaseForm(
            initial={"case_status": d_status_ids, "items_per_page": items_per_page}
        )
...
def query_testcases_from_request(
    request, plan=None
    ...
    tcs = tcs.select_related(
        "author", "default_tester", "case_status", "priority", "category", "reviewer"
    )
