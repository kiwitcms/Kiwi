...
class TestCaseFilter(APITestCase):
    ...
    def test_filter_query_none(self):
        ...
        self.assertIn("setup_duration_sec", result[0])
        self.assertIn("testing_duration_sec", result[0])
        self.assertIn("expected_duration_sec", result[0])
    ...
    def test_duration_properties_in_result(
        self, _name, init_dict, setup_duration_sec, testing_duration_sec, expected_duration_sec
        ):
        testcase = TestCaseFactory(**init_dict)
        result = self.rpc_client.TestCase.filter({"pk": testcase.pk})

        self.assertIsNotNone(result)
        self.assertAlmostEqual(result[0]["setup_duration_sec"], setup_duration_sec)
        self.assertAlmostEqual(result[0]["testing_duration_sec"], testing_duration_sec)
        self.assertAlmostEqual(result[0]["expected_duration_sec"], expected_duration_sec)
