...
class TestXMLSerializer(test.TestCase):
    ...
    def setUpTestData(cls):
        cls.testcase = TestCaseFactory(
            component=[ComponentFactory(), ComponentFactory(), ComponentFactory()]
        )
...
class TestQuerySetBasedSerializer(test.TestCase):
    ...
    def test_get_related_object_pks(self):
        ...
        result = _get_related_object_pks(
    ...
    def test_handle_extra_fields_alias(self):
        ...
        self.assertEqual(
            test_data["default_product_version"], serialize_result["product_version"]
        )
        self.assertEqual(test_data["id"], serialize_result["id"])
        self.assertEqual(
            test_data["product_version"], serialize_result["product_version"]
        )
    ...
    def test_serialize_queryset(self):
        ...
        for plan in serialize_result:
            ...
            self.assertEqual(
                datetime_to_str(expected_plan.create_date), plan["create_date"]
            )
            self.assertEqual(expected_plan.author_id, plan["author_id"])
            self.assertEqual(expected_plan.author.username, plan["author"])
            self.assertEqual(
                expected_plan.product_version_id, plan["product_version_id"]
            )
            self.assertEqual(
                expected_plan.product_version.value, plan["product_version"]
            )
